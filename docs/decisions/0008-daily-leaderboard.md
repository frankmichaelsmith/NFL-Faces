...
- **No email verification** for now: format check only. Registering an email again renames the player and adds a token for the new device; every device the player joined on stays signed in (Frank, 2026-09-09: the game must remember the device). Anyone can claim any address; accepted for speed.
