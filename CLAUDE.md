...
- **Hosting: Vercel** (Frank's existing account), not Cloudflare. **Live at https://nfl-faces.vercel.app** (project `nfl-faces`, team frank-3518, deploys `main` on push; only env var is `VITE_SITE_URL`). Headshots start as static assets under `public/faces/` behind `VITE_IMAGE_BASE_URL`; moving them to object storage later is a config change.
